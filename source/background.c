...
int background_functions(
  ...
  /* non-freestreaming relativistic neutrinos*/
  if (pba->has_ur_fld == _TRUE_) {
    pvecback[pba->index_bg_rho_ur_fld] = pba->Omega0_ur_fld * pow(pba->H0,2) / pow(a_rel,4);
    rho_tot += pvecback[pba->index_bg_rho_ur_fld];
    p_tot += (1./3.) * pvecback[pba->index_bg_rho_ur_fld];
    rho_r += pvecback[pba->index_bg_rho_ur_fld];
  }
...
int background_indices(
  ...
  if (pba->Omega0_ur_fld > 0.)
    pba->has_ur_fld = _TRUE_;
  ...
  /* - index for non-freestreaming ultra-relativistic neutrinos/species */
  class_define_index(pba->index_bg_rho_ur_fld,pba->has_ur_fld,index_bg,1);
...
int background_solve(
  ...
  if (pba->background_verbose > 0) {
    ...
    printf(" -> total Neff = N_ur + N_fld + ... = %f \n",pba->Neff);
  }
...
int background_output_titles(struct background * pba,
  ...
  class_store_columntitle(titles,"(.)rho_ur_fld",pba->has_ur_fld);
...
int background_output_data(
  ...
  for (index_tau=0; index_tau<pba->bt_size; index_tau++){
    ...
    class_store_double(dataptr,pvecback[pba->index_bg_rho_ur_fld],pba->has_ur_fld,storeidx);
