...
int perturb_indices_of_perturbs(
  ...
  ppt->has_source_delta_ur_fld = _FALSE_;
  ...
  ppt->has_source_theta_ur_fld = _FALSE_;
  ...
  for (index_md = 0; index_md < ppt->md_size; index_md++) {
    ...
    if (_scalars_) {
      ...
      if (ppt->has_density_transfers == _TRUE_) {
        ...
        if (pba->has_ur_fld == _TRUE_)
          ppt->has_source_delta_ur_fld = _TRUE_;
      ...
      if (ppt->has_velocity_transfers == _TRUE_) {
        ...
        if (pba->has_ur_fld == _TRUE_)
          ppt->has_source_theta_ur_fld = _TRUE_;
        if (pba->has_dr == _TRUE_)
      ...
      class_define_index(ppt->index_tp_delta_ur_fld,   ppt->has_source_delta_ur_fld,  index_type,1);
      ...
      class_define_index(ppt->index_tp_theta_ur_fld,   ppt->has_source_theta_ur_fld,  index_type,1);
...
int perturb_prepare_output(struct background * pba,
  ...
  if (ppt->k_output_values_num > 0) {
    ...
    if (ppt->has_scalars == _TRUE_){
      ...
      /* Non-freestreaming Ultrarelativistic species */
      class_store_columntitle(ppt->scalar_titles,"delta_ur_fld",pba->has_ur_fld);
      class_store_columntitle(ppt->scalar_titles,"theta_ur_fld",pba->has_ur_fld);
      /* Cold dark matter */
...
int perturb_vector_init(
  ...
  if (_scalars_) {
    ...
    /* non-freestreaming ultra relativistic neutrinos */

    if (pba->has_ur_fld) {

      class_define_index(ppv->index_pt_delta_ur_fld,_TRUE_,index_pt,1); /* density of non-freestreaming ultra-relativistic neutrinos/relics */
      class_define_index(ppv->index_pt_theta_ur_fld,_TRUE_,index_pt,1); /* velocity of non-freestreaming ultra-relativistic neutrinos/relics */
  ...
  /** - case of switching approximation while a wavenumber is being integrated */
    ...
    if (_scalars_) {
      ...
      if (pba->has_ur_fld == _TRUE_) {
        ...
        ppv->y[ppv->index_pt_delta_ur_fld] =
          ppw->pv->y[ppw->pv->index_pt_delta_ur_fld];

        ppv->y[ppv->index_pt_theta_ur_fld] =
          ppw->pv->y[ppw->pv->index_pt_theta_ur_fld];
      }
...
int perturb_initial_conditions(struct precision * ppr,
  ...
  if (_scalars_) {
    ...
    if (pba->has_ur_fld == _TRUE_) {
      rho_r += ppw->pvecback[pba->index_bg_rho_ur_fld];
      // rho_nu only includes collisionless species!
      //rho_nu += ppw->pvecback[pba->index_bg_rho_ur_fld];
    }
    ...
    */
    ...
    if ((ppt->has_ad == _TRUE_) && (index_ic == ppt->index_ic_ad)) {
      ...
      /* all free-streaming relativistic relics: ur, early ncdm, dr */
    ...
    if (pba->has_ur_fld == _TRUE_) {
      ...
      ppw->pv->y[ppw->pv->index_pt_delta_ur_fld] = ppw->pv->y[ppw->pv->index_pt_delta_g];

      ppw->pv->y[ppw->pv->index_pt_theta_ur_fld] = ppw->pv->y[ppw->pv->index_pt_theta_g];
...
int perturb_total_stress_energy(
  ...
  double theta_ur=0.;
  double shear_ur=0.;
  double delta_ur_fld=0.;
  double theta_ur_fld=0.;
  ...
  if (_scalars_) {
    ...
    /* non-freestreaming ultra-relativistic neutrino/relics contribution */

    if (pba->has_ur_fld == _TRUE_) {
      delta_ur_fld = y[ppw->pv->index_pt_delta_ur_fld];
      theta_ur_fld = y[ppw->pv->index_pt_theta_ur_fld];
      ppw->delta_rho += ppw->pvecback[pba->index_bg_rho_ur_fld]*delta_ur_fld;
      ppw->rho_plus_p_theta += 4./3.*ppw->pvecback[pba->index_bg_rho_ur_fld]*theta_ur_fld;
      ppw->delta_p += 1./3.*ppw->pvecback[pba->index_bg_rho_ur_fld]*delta_ur_fld;
      rho_plus_p_tot += 4./3. * ppw->pvecback[pba->index_bg_rho_ur_fld];
    }
...
int perturb_sources(
  ...
  if (_scalars_) {
    ...
    /* delta_ur_fld */
    if (ppt->has_source_delta_ur_fld == _TRUE_) {
      _set_source_(ppt->index_tp_delta_ur_fld) = y[ppw->pv->index_pt_delta_ur_fld];
    }
    ...
    /* theta_ur_fld */
    if (ppt->has_source_theta_ur_fld == _TRUE_) {
      _set_source_(ppt->index_tp_theta_ur_fld) = y[ppw->pv->index_pt_theta_ur_fld];
    }
...
int perturb_print_variables(double tau,
  ...
  double delta_ur_fld=0.,theta_ur_fld=0.;
  ...
  if (_scalars_) {
    ...
    if (ppt->gauge == synchronous) {
      ...
      if (pba->has_ur_fld == _TRUE_) {
        delta_ur_fld -= 4. * pvecback[pba->index_bg_H]*pvecback[pba->index_bg_a]*alpha;
        theta_ur_fld += k*k*alpha;
      }
    ...
    /* Non-freestreaming Ultra relativistic species */
    class_store_double(dataptr, delta_ur_fld, pba->has_ur_fld, storeidx);
    class_store_double(dataptr, theta_ur_fld, pba->has_ur_fld, storeidx);
    /* Cold dark matter */
...
int perturb_derivs(double tau,
  ...
  if (_scalars_) {
    ...
    /** - ---> non-freestreaming ultra-relativistic neutrino/relics (ur) */

    if (pba->has_ur_fld == _TRUE_) {

        /** - -----> ur_fld density */
        dy[pv->index_pt_delta_ur_fld] = -4./3.*(y[pv->index_pt_theta_ur_fld] + metric_continuity);

        /** - -----> ur_fld velocity */
        dy[pv->index_pt_theta_ur_fld] = k2*y[pv->index_pt_delta_ur_fld]/4. + metric_euler;
    }
