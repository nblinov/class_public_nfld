...
int input_read_parameters(
  ...
  /** - Omega0_ur_fld (non-freestreaming ultra-relativistic species) */

  class_call(parser_read_double(pfc,"N_fld",&param1,&flag1,errmsg),
             errmsg,
             errmsg);
  if (flag1 == _TRUE_)
  {
    pba->Omega0_ur_fld = param1*7./8.*pow(4./11.,4./3.)*pba->Omega0_g;
    Omega_tot += pba->Omega0_ur_fld;
  }
  else
    pba->Omega0_ur_fld = 0.;
